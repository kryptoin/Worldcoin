...
class SSLIOStreamDevice : public boost::iostreams::device<boost::iostreams::bidirectional> {
public:
    SSLIOStreamDevice(boost::asio::io_context& io_context_in, boost::asio::ssl::stream<typename Protocol::socket> &streamIn, bool fUseSSLIn)
        : io_context(io_context_in), stream(streamIn), fUseSSL(fUseSSLIn), fNeedHandshake(fUseSSLIn) {}
    ...
    bool connect(const std::string& server, const std::string& port) {
        ...
        //     endpoints = resolver.resolve(query, error);
        // } catch (boost::system::system_error& e) {
        //     tcp::resolver::query query(server, port, tcp::resolver::query::flags());
        //     endpoints = resolver.resolve(query, error);
        // }
...
try {
    endpoints = resolver.resolve(server, port, error);
} catch (boost::system::system_error& e) {
    // Optional fallback, but modern Boost handles DNS fallback internally
    endpoints = resolver.resolve(server, port, error);
}
